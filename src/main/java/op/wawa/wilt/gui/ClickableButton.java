...
public abstract class ClickableButton {
    ...
    public void drawScreen() {
        ...
        Minecraft.getMinecraft().fontRendererObj.drawStringWithShadow(text, x - Minecraft.getMinecraft().fontRendererObj.getStringWidth(text) / 2F, startY + (height == 20 ? 6 : 2), new Color(216, 216, 216).getRGB());
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            ...
            int startY = y - height / 2;
            int endY = y + height / 2;
            if (RenderUtils.isHovered(startX, startY, endX, endY, mouseX, mouseY)) {
                clicked();
            }
        }
    }
