...
public class AsyncVersionSlider extends GuiButton {
    private float dragValue = (float) (ProtocolCollection.values().length - Arrays.asList(ProtocolCollection.values()).indexOf(ProtocolCollection.getProtocolCollectionById(ViaForge.NATIVE_VERSION.getVersion()))) / ProtocolCollection.values().length;
    ...
    protected void mouseDragged(Minecraft mc, int mouseX, int mouseY) {
        if (this.visible) {
            if (this.dragging) {
                ...
                this.displayString = values[(int) (this.sliderValue * (values.length - 1))].getVersion().getName();
                ViaForge.targetVersion = VersionEnum.fromProtocolVersion(values[(int) (this.sliderValue * (values.length - 1))].getVersion());
            }
